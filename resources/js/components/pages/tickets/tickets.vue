<template>
    ...
    <PageLayout :pageName="title">
        ...
        <div class="scroll-style mb-3 w-full overflow-auto flex items-center justify-start">
            <div :class="activeStatus == 'FOR_REVIEW' ? 'opacity-100' : 'opacity-40'" class="mr-3 min-w-160 w-1/5">
                <ButtonCard @click="getTickets('FOR_REVIEW', 0)" link="/tickets" arrow="false" cardTitle="For Review" value="10" icon="/img/icon/eye-active.png" />
            </div>
            <div :class="activeStatus == 'ACKNOWLEDGED' ? 'opacity-100' : 'opacity-40'" class="mr-3 min-w-160 w-1/5">
                <ButtonCard @click="getTickets('ACKNOWLEDGED', 0)" link="/tickets" arrow="false" cardTitle="Acknowledged" value="10" icon="/img/icon/like-active.png" />
            </div>
            <div :class="activeStatus == 'ON_GOING' ? 'opacity-100' : 'opacity-40'" class="mr-3 min-w-160 w-1/5">
                <ButtonCard @click="getTickets('ON_GOING', 0)" link="/tickets" arrow="false" cardTitle="On-going" value="10" icon="/img/icon/clock-active.png" />
            </div>
            <div :class="activeStatus == 'RESOLVED' ? 'opacity-100' : 'opacity-40'" class="mr-3 min-w-160 w-1/5">
                <ButtonCard @click="getTickets('RESOLVED', 0)" link="/tickets" arrow="false" cardTitle="Resolved" value="10" icon="/img/icon/party-active.png" />
            </div>
            <div :class="activeStatus == 'INVALID' ? 'opacity-100' : 'opacity-40'" class="mr-3 min-w-160 w-1/5">
                <ButtonCard @click="getTickets('INVALID', 0)" link="/tickets" arrow="false" cardTitle="Invalid" value="10" icon="/img/icon/warning-active.png" />
            </div>
        </div>
...
export default {
    ...
    components: { PageLayout, ButtonCard, ContentCard, Modal, AlertTop},
    async mounted() {
    ...
    methods: {
        ...
        async getTickets(val, page){
            ...
            if(this.activeTab == 'All'){
                this.tickets = responseFiltered.filter((a) => (a.status == this.activeStatus));
            }else if(this.activeTab == 'My Agency'){
                this.tickets = responseFiltered.filter((a) => (a.status == this.activeStatus && a.assigned_agencies['0'].data.code == "DENR"));
            }else if(this.activeTab == 'Following'){
                this.tickets = responseFiltered.filter((a) => (a.status == this.activeStatus && a.isFollow == 1));
            }
            //Pagination
