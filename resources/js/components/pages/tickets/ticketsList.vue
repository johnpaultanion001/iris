<template>
  ...
  <div class="grid grid-cols-12 gap-3 mb-6 pt-4">
    <div class="col-span-11 md:col-span-12 xl:col-span-6">
      <div class="ticket-tabs bg-grey p-1 rounded-xl flex">
        <div v-if="userRole == 'SUPER_ADMIN'" @click="activeTab = 'All'; getTickets(activeStatus, 0); getAllTickets();"
             :class="activeTab === 'All' ? 'bg-white  shadow-third' : 'bg-transparent'"
             class="rounded-lg w-1/3 text-center">
          <p class="text-xs sm:text-base truncate text-blue-grey font-inter-400 px-1 py-1.5">All</p>
        </div>
        <div @click="activeTab = 'My Agency'; getTickets(activeStatus, 0); getAllTickets();"
             :class="activeTab === 'My Agency' ? 'bg-white  shadow-third' : 'bg-transparent', userRole == 'SUPER_ADMIN' ? 'w-1/3' : 'w-1/2'"
             class="rounded-lg text-center">
          <p class="text-xs sm:text-base truncate text-blue-grey font-inter-400 px-1 py-1.5">My Agency</p>
        </div>
        <div @click="activeTab = 'Following'; getTickets(activeStatus, 0); getAllTickets();"
             :class="activeTab === 'Following' ? 'bg-white  shadow-third' : 'bg-transparent', userRole == 'SUPER_ADMIN' ? 'w-1/3' : 'w-1/2'"
             class="rounded-lg text-center">
          <p class="text-xs sm:text-base truncate text-blue-grey font-inter-400 px-1 py-1.5">Following
            ({{ alltickets.filter((a) => (a.isFollow == 1)).length }})</p>
        </div>
      </div>
    </div>
  ...
  <div class="scroll-style w-full overflow-auto flex items-center justify-start pb-3">
    <div :class="activeStatus == 'FOR_REVIEW' ? 'opacity-100' : 'opacity-40'" class="mr-3 min-w-160 w-1/5">
      <ButtonCard @click="getTickets('FOR_REVIEW', 0)" link="" arrow="false" cardTitle="For Review"
                  :value="alltickets.filter((a) => (a.status == 'FOR_REVIEW')).length"
                  :active="activeStatus == 'FOR_REVIEW' ? 'true' : 'false'">
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M8.80002 8.7999C8.80002 7.47442 9.87454 6.3999 11.2 6.3999C12.5255 6.3999 13.6 7.47442 13.6 8.7999C13.6 10.1254 12.5255 11.1999 11.2 11.1999C10.537 11.1999 9.9386 10.9326 9.50297 10.497C9.06734 10.0613 8.80002 9.46298 8.80002 8.7999Z"
            fill="#1267E6"/>
          <path fill-rule="evenodd" clip-rule="evenodd"
                d="M10 19.5999C15.302 19.5999 19.6 15.3018 19.6 9.9999C19.6 4.69797 15.302 0.399902 10 0.399902C4.69809 0.399902 0.400024 4.69797 0.400024 9.9999C0.400024 15.3018 4.69809 19.5999 10 19.5999ZM11.2 3.9999C8.54906 3.9999 6.40002 6.14894 6.40002 8.7999C6.40002 9.68956 6.64278 10.5236 7.06478 11.2381L4.3515 13.9514C3.88287 14.42 3.88287 15.1798 4.3515 15.6484C4.82013 16.1171 5.57992 16.1171 6.04855 15.6484L8.76184 12.9351C9.47637 13.3572 10.3104 13.5999 11.2 13.5999C13.851 13.5999 16 11.4509 16 8.7999C16 6.14894 13.851 3.9999 11.2 3.9999Z"
                fill="#1267E6"/>
        </svg>
      </ButtonCard>
    </div>
    <div :class="activeStatus == 'ACKNOWLEDGED' ? 'opacity-100' : 'opacity-40'" class="mr-3 min-w-160 w-1/5">
      <ButtonCard @click="getTickets('ACKNOWLEDGED', 0)" link="" arrow="false" cardTitle="Acknowledged"
                  :value="alltickets.filter((a) => (a.status == 'ACKNOWLEDGED')).length"
                  :active="activeStatus == 'ACKNOWLEDGED' ? 'true' : 'false'">
        <svg width="20" height="18" viewBox="0 0 20 18" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd"
                d="M7.90835 1.71849C8.82588 0.0873266 11.1744 0.0873257 12.0919 1.71849L18.7883 13.6231C19.6882 15.223 18.5321 17.1998 16.6965 17.1998H3.30376C1.46818 17.1998 0.312066 15.223 1.21198 13.6231L7.90835 1.71849ZM11.2 13.5999C11.2 14.2626 10.6628 14.7999 10 14.7999C9.33728 14.7999 8.80002 14.2626 8.80002 13.5999C8.80002 12.9372 9.33728 12.3999 10 12.3999C10.6628 12.3999 11.2 12.9372 11.2 13.5999ZM10 3.9999C9.33728 3.9999 8.80002 4.53716 8.80002 5.1999V8.7999C8.80002 9.46264 9.33728 9.9999 10 9.9999C10.6628 9.9999 11.2 9.46264 11.2 8.7999V5.1999C11.2 4.53716 10.6628 3.9999 10 3.9999Z"
                fill="#1267E6"/>
        </svg>
      </ButtonCard>
    </div>
    <div :class="activeStatus == 'ON_GOING' ? 'opacity-100' : 'opacity-40'" class="mr-3 min-w-160 w-1/5">
      <ButtonCard @click="getTickets('ON_GOING', 0)" link="" arrow="false" cardTitle="On-going"
                  :value="alltickets.filter((a) => (a.status == 'ON_GOING')).length"
                  :active="activeStatus == 'ON_GOING' ? 'true' : 'false'">
        <svg width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd"
                d="M0.599976 4.2001C0.599976 2.21187 2.21175 0.600098 4.19997 0.600098H16.2C16.6545 0.600098 17.07 0.856901 17.2733 1.26344C17.4765 1.66998 17.4327 2.15648 17.16 2.5201L14.1 6.6001L17.16 10.6801C17.4327 11.0437 17.4765 11.5302 17.2733 11.9368C17.07 12.3433 16.6545 12.6001 16.2 12.6001H4.19997C3.53723 12.6001 2.99997 13.1374 2.99997 13.8001V17.4001C2.99997 18.0628 2.46271 18.6001 1.79997 18.6001C1.13723 18.6001 0.599976 18.0628 0.599976 17.4001V4.2001Z"
                fill="#1267E6"/>
        </svg>
      </ButtonCard>
    </div>
    <div :class="activeStatus == 'RESOLVED' ? 'opacity-100' : 'opacity-40'" class="mr-3 min-w-160 w-1/5">
      <ButtonCard @click="getTickets('RESOLVED', 0)" link="" arrow="false" cardTitle="Resolved"
                  :value="alltickets.filter((a) => (a.status == 'RESOLVED')).length"
                  :active="activeStatus == 'RESOLVED' ? 'true' : 'false'">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
          <path fill-rule="evenodd" clip-rule="evenodd"
                d="M9.99997 0.399902C10.6627 0.399902 11.2 0.937161 11.2 1.5999V3.18746L15.9453 5.0856L17.8633 4.12659C18.4561 3.8302 19.1769 4.07047 19.4733 4.66325C19.7697 5.25602 19.5294 5.97683 18.9366 6.27322L17.4568 7.01311L19.5426 13.5167C19.6873 13.9677 19.5531 14.4615 19.2003 14.7774C18.3518 15.5367 17.2286 15.9999 16 15.9999C14.7714 15.9999 13.6481 15.5367 12.7997 14.7774C12.4468 14.4615 12.3127 13.9677 12.4573 13.5167L14.5157 7.09862L11.2 5.77234V17.1999H13.6C14.2627 17.1999 14.8 17.7372 14.8 18.3999C14.8 19.0626 14.2627 19.5999 13.6 19.5999H6.39997C5.73723 19.5999 5.19997 19.0626 5.19997 18.3999C5.19997 17.7372 5.73723 17.1999 6.39997 17.1999H8.79997V5.77234L5.48427 7.09862L7.54264 13.5167C7.68727 13.9677 7.55315 14.4615 7.20027 14.7774C6.35186 15.5367 5.22857 15.9999 3.99998 15.9999C2.77138 15.9999 1.6481 15.5367 0.799685 14.7774C0.446804 14.4615 0.312682 13.9677 0.45731 13.5167L2.54311 7.01311L1.06332 6.27322C0.470551 5.97683 0.230283 5.25602 0.526669 4.66325C0.823056 4.07047 1.54386 3.8302 2.13664 4.12659L4.05464 5.0856L8.79997 3.18746V1.5999C8.79997 0.937161 9.33723 0.399902 9.99997 0.399902ZM3.99998 10.3293L3.01809 13.3908C3.31759 13.5253 3.64961 13.5999 3.99998 13.5999C4.35034 13.5999 4.68236 13.5253 4.98186 13.3908L3.99998 10.3293ZM16 10.3293L15.0181 13.3908C15.3176 13.5253 15.6496 13.5999 16 13.5999C16.3503 13.5999 16.6823 13.5253 16.9818 13.3908L16 10.3293Z"
                fill="#1267E6"/>
        </svg>
      </ButtonCard>
    </div>
    <div :class="activeStatus == 'INVALID' ? 'opacity-100' : 'opacity-40'" class="mr-3 min-w-160 w-1/5">
      <ButtonCard @click="getTickets('INVALID', 0)" link="" arrow="false" cardTitle="Invalid"
                  :value="alltickets.filter((a) => (a.status == 'INVALID')).length"
                  :active="activeStatus == 'INVALID' ? 'true' : 'false'">
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd"
                d="M19.6 9.9999C19.6 15.3018 15.3019 19.5999 9.99999 19.5999C4.69806 19.5999 0.399994 15.3018 0.399994 9.9999C0.399994 4.69797 4.69806 0.399902 9.99999 0.399902C15.3019 0.399902 19.6 4.69797 19.6 9.9999ZM14.1719 15.8687C12.9949 16.7069 11.555 17.1999 9.99999 17.1999C6.02354 17.1999 2.79999 13.9764 2.79999 9.9999C2.79999 8.44487 3.29297 7.00497 4.13116 5.82798L14.1719 15.8687ZM15.869 14.1717L5.82824 4.13094C7.00521 3.29283 8.44504 2.7999 9.99999 2.7999C13.9764 2.7999 17.2 6.02345 17.2 9.9999C17.2 11.5549 16.7071 12.9947 15.869 14.1717Z"
                fill="#1267E6"/>
        </svg>
      </ButtonCard>
    </div>
      ...
      <ContentCard class="py-4">
        <div class="grid grid-cols-2 gap-2 pb-4 px-4">
          <div class="col-span-1 flex items-center">
            <div @click="filterItemNumberDropdown = !filterItemNumberDropdown"
                 class="mr-1 py-1.5 px-2.5 cursor-pointer relative bg-white text-xxs text-black font-opensans-600 border border-light rounded flex items-center">
              {{ perpage }} <img src="/img/icon/filter.svg" class="ml-1">
              <div v-if="filterItemNumberDropdown" style="top:28px;right:-1px;left:-1px;"
                   class="z-9 dropdown absolute border border-light rounded-b-lg bg-white shadow-secondary overflow-hidden">
          ...
          <div class="col-span-1 flex justify-end relative">
            <input v-model="filterSearch" type="text"
                   class="py-1.5 px-3 text-xxs rounded-lg border border-light focus-visible:border-blue-grey"
                   placeholder="Search">
          </div>
        </div>
        <div class="scroll-style overflow-auto mb-4">
          <table class="table-auto w-full">
            ...
            <tr class="border-b border-light">
              ...
              <th class="p-2.5">
                <button @click="ticketsOrder = 'ticket_no'; ticketsASC = !ticketsASC; orderedTickets();"
                        class="filter-btn">Ticket No.<img src="/img/icon/filter.svg" class="ml-1.5"></button>
              </th>
              <th class="p-2.5 text-center">
                <button
                  @click="ticketsOrder = 'first_name'; ticketsOrderArray = 'reported_by'; ticketsASC = !ticketsASC; orderedTickets();"
                  class="filter-btn">Reported By<img src="/img/icon/filter.svg" class="ml-1.5"></button>
              </th>
              <th class="p-2.5 text-center">
                <button @click="ticketsOrder = 'product_service'; ticketsASC = !ticketsASC; orderedTickets();"
                        class="filter-btn">Product/Service<img src="/img/icon/filter.svg" class="ml-1.5"></button>
              </th>
              <th v-if="colVendor != 'off'" class="p-2.5 text-center">
                <button
                  @click="ticketsOrder = 'vendor_name'; ticketsOrderArray = 'vendor'; ticketsASC = !ticketsASC; orderedTickets();"
                  class="filter-btn">Vendor Name<img src="/img/icon/filter.svg" class="ml-1.5"></button>
              </th>
              <th class="p-2.5 text-center">
                <button @click="ticketsOrder = 'complaint'; ticketsASC = !ticketsASC; orderedTickets();"
                        class="filter-btn">Complaint<img src="/img/icon/filter.svg" class="ml-1.5"></button>
              </th>
              <th class="p-2.5 text-center">
                <button @click="ticketsOrder = 'assigned_agencies'; ticketsASC = !ticketsASC; orderedTickets();"
                        class="filter-btn">Assigned Agency<img src="/img/icon/filter.svg" class="ml-1.5"></button>
              </th>
              <th v-if="colSeverity != 'off'" class="p-2.5 text-center">
                <button @click="ticketsOrder = 'severity'; ticketsASC = !ticketsASC; orderedTickets();"
                        class="filter-btn">Severity<img src="/img/icon/filter.svg" class="ml-1.5"></button>
              </th>
              <th class="p-2.5 text-center">
                <button
                  @click="ticketsOrder = 'created_at'; ticketsOrderArray = 'reported_by'; ticketsASC = !ticketsASC; orderedTickets();"
                  class="filter-btn">Date Submitted<img src="/img/icon/filter.svg" class="ml-1.5"></button>
              </th>
              <th class="p-2.5 text-center">
            ...
            <tr class="border-b border-light relative">
              <td class="w-12 min-w-44">
                <router-link :to="'/ticket-information/'+ticket.ticket_no">
                  <svg xmlns="http://www.w3.org/2000/svg" width="21" height="15" viewBox="0 0 21 15" fill="none"
                       class="m-auto block">
                    <path
                      d="M10.5 9.5C11.6046 9.5 12.5 8.60457 12.5 7.5C12.5 6.39543 11.6046 5.5 10.5 5.5C9.39544 5.5 8.50001 6.39543 8.50001 7.5C8.50001 8.60457 9.39544 9.5 10.5 9.5Z"
                      fill="#1267E5"/>
                    <path fill-rule="evenodd" clip-rule="evenodd"
                          d="M0.957764 7.50004C2.23202 3.44291 6.02232 0.5 10.5 0.5C14.9776 0.5 18.7679 3.44288 20.0422 7.49996C18.7679 11.5571 14.9776 14.5 10.4999 14.5C6.02232 14.5 2.23204 11.5571 0.957764 7.50004ZM14.5 7.5C14.5 9.70914 12.7091 11.5 10.5 11.5C8.29087 11.5 6.50001 9.70914 6.50001 7.5C6.50001 5.29086 8.29087 3.5 10.5 3.5C12.7091 3.5 14.5 5.29086 14.5 7.5Z"
                          fill="#1267E5"/>
                  </svg>
                </router-link>
              </td>
              <td class="w-12 min-w-44">
                <svg v-if="ticket.isFollow == true"
                     @click.prevent="modalTicketID = ticket.ticket_no; openModal('unfollow')"
                     class="cursor-pointer m-auto block" xmlns="http://www.w3.org/2000/svg" width="11" height="17"
                     viewBox="0 0 11 17" fill="none">
                  <path
                    d="M0.5 2.5C0.5 1.39543 1.39543 0.5 2.5 0.5H8.5C9.60457 0.5 10.5 1.39543 10.5 2.5V16.5L5.5 14L0.5 16.5V2.5Z"
                    fill="#1267E5"/>
                </svg>
                <svg v-if="ticket.isFollow == false"
                     @click.prevent="modalTicketID = ticket.ticket_no; openModal('follow')"
                     class="cursor-pointer m-auto block" xmlns="http://www.w3.org/2000/svg" width="11" height="17"
                     viewBox="0 0 11 17" fill="none">
                  <path
                    d="M5.27639 13.5528L1 15.691V2.5C1 1.67157 1.67157 1 2.5 1H8.5C9.32843 1 10 1.67157 10 2.5V15.691L5.72361 13.5528L5.5 13.441L5.27639 13.5528Z"
                    stroke="#1267E5"/>
                </svg>
              </td>
              ...
              <td v-if="colVendor != 'off'" class="p-2.5 font-opensans-600 text-xxxs text-dark2"
                  v-for="(item, index) in ticket.vendor" ref="tickets">
              ...
              <td class="p-2.5 font-opensans-600 text-xxxs text-white flex items-center whitespace-nowrap">
                <div class="flex items-center whitespace-nowrap" v-for="(item, index) in ticket.assigned_agencies"
                     ref="tickets">
                  <div class="whitespace-nowrap" ref="tickets">
                    <span v-if="index <= 3" :style="'background-color:'+ item.color +';'"
                          class="px-2 py-1 rounded-full mr-1 inline-block text-center w-auto min-w-52">{{
                        item.code
                      }}</span>
                    <span v-if="index == 3" style="background-color: #54A581"
                          class="px-2 py-1 rounded-full mr-1 inline-block text-center">+{{
                        ticket.assigned_agencies.length - 4
                      }}</span>
                  </div>
                </div>
              </td>
              <td v-if="colSeverity != 'off'" class="p-2.5 font-opensans-600 text-xxxs text-white text-center">
                                    <span
                                      :style="ticket.severity === 'LOW' ? 'background-color: #FFD600;' : '' || ticket.severity === 'MEDIUM' ? 'background-color: #F2994A;' : '' || ticket.severity === 'HIGH' ? 'background-color: #FA3A0E;' : ''"
                                      class="px-2 py-1 rounded mr-1 w-auto min-w-52">
                                        {{ ticket.severity }}
                                    </span>
              </td>
        ...
        <div class="pagination-cont flex items-center justify-between px-4">
          ...
          <div v-if="tickets.length > perpage" class="flex items-center justify-end">
            <button @click="pageNumber >  0 ? updatePageNumber(pageNumber - 1) : null"
                    :class="pageNumber >  0 ? 'bg-white hover:bg-lightergrey' : 'opacity-40 bg-lightergrey hover:bg-lightergrey'"
                    class="mx-1 b-white hover:bg-lightergrey w-9 h-9 border border-lightgrey flex items-center justify-center rounded-full">
              <img src="/img/icon/previous.svg">
            </button>
            <button @click="updatePageNumber(0)"
                    :class="pageNumber == 0 ? 'bg-blue hover:bg-blue text-white border-blue' : 'b-white hover:bg-lightergrey text-darkgrey border-lightergrey'"
                    class="mx-1 font-inter-400 text-sm w-9 h-9 border flex items-center justify-center rounded-full">
              1
            </button>
            <p v-if="pageNumber > (pageRange/2)">...</p>
            <div v-for="pageIndex in paginateTotal" @click.prevent="updatePageNumber(pageIndex - 1)">
              <button
                v-if="(pageIndex > pageStart && pageIndex < pageEnd && pageIndex != 1 && pageIndex != paginateTotal)"
                :class="(pageIndex - 1) == pageNumber ? 'bg-blue hover:bg-blue text-white border-blue' : 'b-white hover:bg-lightergrey text-darkgrey border-lightergrey'"
                class="mx-1 font-inter-400 text-sm w-9 h-9 border flex items-center justify-center rounded-full">
                {{ pageIndex }}
              </button>
            </div>
            <p v-if="pageNumber < (paginateTotal - (pageRange/2))">...</p>
            <button @click="updatePageNumber(paginateTotal - 1)"
                    :class="pageNumber == (paginateTotal - 1) ? 'bg-blue hover:bg-blue text-white border-blue' : 'b-white hover:bg-lightergrey text-darkgrey border-lightergrey'"
                    class="mx-1 font-inter-400 text-sm w-9 h-9 border flex items-center justify-center rounded-full">
              {{ paginateTotal }}
            </button>
            <button @click="(paginateTotal - 1) > pageNumber ? updatePageNumber(pageNumber + 1) : null"
                    :class="(paginateTotal - 1) > pageNumber ? 'bg-white hover:bg-lightergrey' : 'opacity-40 bg-lightergrey hover:bg-lightergrey'"
                    class="mx-1 b-white hover:bg-lightergrey w-9 h-9 border border-lightergrey flex items-center justify-center rounded-full">
              <img src="/img/icon/next.svg">
            </button>
          </div>
        </div>
        <div v-click-out-side="closeAction" style="top: 103px;"
             class="action-btn absolute right-0 bg-white shadow-secondary">
          <!-- <div class="py-5 border-b border-b-light px-6 flex items-center">
          </div> -->
          <div v-for="(ticket, index) in orderedTickets()" ref="tickets">
            <div @click="openAction(ticket.ticket_no)"
                 class="cursor-pointer relative h-15 border-b border-b-light px-6 flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" width="5" height="17" viewBox="0 0 5 17" fill="none">
                <path
                  d="M2.5 4.5C1.39543 4.5 0.5 3.60457 0.5 2.5C0.5 1.39543 1.39543 0.5 2.5 0.5C3.60457 0.5 4.5 1.39543 4.5 2.5C4.5 3.60457 3.60457 4.5 2.5 4.5Z"
                  fill="#525F7F"/>
                <path
                  d="M2.5 10.5C1.39543 10.5 0.5 9.60457 0.5 8.5C0.5 7.39543 1.39543 6.5 2.5 6.5C3.60457 6.5 4.5 7.39543 4.5 8.5C4.5 9.60457 3.60457 10.5 2.5 10.5Z"
                  fill="#525F7F"/>
                <path
                  d="M2.5 16.5C1.39543 16.5 0.5 15.6046 0.5 14.5C0.5 13.3954 1.39543 12.5 2.5 12.5C3.60457 12.5 4.5 13.3954 4.5 14.5C4.5 15.6046 3.60457 16.5 2.5 16.5Z"
                  fill="#525F7F"/>
              </svg>
              <div v-if="actionActive && showAction == ticket.ticket_no"
                   class="action-card shadow-secondary absolute rounded-lg bg-white">
                <router-link :to="'ticket/'+ticket.ticket_no">
                  <div
                    class="cursor-pointer whitespace-nowrap py-3 px-4 flex items-center font-inter-400 text-black text-sm hover:bg-lighter">
                                        <span class="w-5 mr-3 text-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="15"
                                                 viewBox="0 0 20 15" fill="none" class="mx-auto">
                                            <path
                                              d="M10 9.5C11.1046 9.5 12 8.60457 12 7.5C12 6.39543 11.1046 5.5 10 5.5C8.89544 5.5 8.00001 6.39543 8.00001 7.5C8.00001 8.60457 8.89544 9.5 10 9.5Z"
                                              fill="#525F7F"/>
                                            <path fill-rule="evenodd" clip-rule="evenodd"
                                                  d="M0.457764 7.50004C1.73202 3.44291 5.52232 0.5 9.99997 0.5C14.4776 0.5 18.2679 3.44288 19.5422 7.49996C18.2679 11.5571 14.4776 14.5 9.99995 14.5C5.52232 14.5 1.73204 11.5571 0.457764 7.50004ZM14 7.5C14 9.70914 12.2091 11.5 10 11.5C7.79087 11.5 6.00001 9.70914 6.00001 7.5C6.00001 5.29086 7.79087 3.5 10 3.5C12.2091 3.5 14 5.29086 14 7.5Z"
                                                  fill="#525F7F"/>
                                            </svg>
                                        </span><span>View</span>
                  </div>
                ...
                <div
                  @click.prevent="modalTicketID = ticket.ticket_no; ticket.isFollow ? openModal('unfollow') : openModal('follow')"
                  class="cursor-pointer whitespace-nowrap py-3 px-4 flex items-center font-inter-400 text-black text-sm hover:bg-lighter">
                                    <span class="w-5 mr-3 text-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="17"
                                             viewBox="0 0 10 17" fill="none" class="mx-auto">
                                        <path
                                          d="M0 2.5C0 1.39543 0.895431 0.5 2 0.5H8C9.10457 0.5 10 1.39543 10 2.5V16.5L5 14L0 16.5V2.5Z"
                                          fill="#525F7F"/>
                                        </svg>
                                    </span><span>{{ ticket.isFollow ? 'Unfollow' : 'Follow' }}</span>
                </div>
                <div @click.prevent="modalTicketID = ticket.ticket_no; openModal('ticketStatusModal')"
                     class="cursor-pointer whitespace-nowrap py-3 px-4 flex items-center font-inter-400 text-black text-sm hover:bg-lighter">
                                    <span class="w-5 mr-3 text-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="17"
                                             viewBox="0 0 14 17" fill="none" class="mx-auto">
                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                              d="M1 0.5C1.55228 0.5 2 0.947715 2 1.5V3.60125C3.27009 2.30489 5.04052 1.5 7 1.5C10.0494 1.5 12.641 3.44932 13.6014 6.16675C13.7855 6.68747 13.5126 7.25879 12.9918 7.44284C12.4711 7.62689 11.8998 7.35396 11.7157 6.83325C11.0289 4.88991 9.17547 3.5 7 3.5C5.36507 3.5 3.91204 4.28502 2.99935 5.5H6C6.55228 5.5 7 5.94772 7 6.5C7 7.05228 6.55228 7.5 6 7.5H1C0.447715 7.5 0 7.05228 0 6.5V1.5C0 0.947715 0.447715 0.5 1 0.5ZM1.00817 9.55716C1.52888 9.37311 2.1002 9.64604 2.28425 10.1668C2.97112 12.1101 4.82453 13.5 7 13.5C8.63493 13.5 10.088 12.715 11.0006 11.5L8 11.5C7.44772 11.5 7 11.0523 7 10.5C7 9.94771 7.44772 9.5 8 9.5H13C13.2652 9.5 13.5196 9.60536 13.7071 9.79289C13.8946 9.98043 14 10.2348 14 10.5V15.5C14 16.0523 13.5523 16.5 13 16.5C12.4477 16.5 12 16.0523 12 15.5V13.3987C10.7299 14.6951 8.95948 15.5 7 15.5C3.95059 15.5 1.35905 13.5507 0.398572 10.8332C0.214525 10.3125 0.487449 9.74121 1.00817 9.55716Z"
                                              fill="#525F7F"/>
                                        </svg>
                                    </span><span>Update Ticket Status</span>
                </div>
                <router-link v-if="activeStatus != 'RESOLVED' && activeStatus != 'INVALID'"
                             :to="'/edit-ticket/'+ticket.ticket_no">
                  <div
                    class="cursor-pointer whitespace-nowrap py-3 px-4 flex items-center font-inter-400 text-black text-sm hover:bg-lighter">
                                        <span class="w-5 mr-3 text-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="15"
                                                 viewBox="0 0 14 15" fill="none" class="mx-auto">
                                            <path
                                              d="M10.5858 1.08579C11.3668 0.304738 12.6332 0.304738 13.4142 1.08579C14.1953 1.86683 14.1953 3.13316 13.4142 3.91421L12.6213 4.70711L9.79289 1.87868L10.5858 1.08579Z"
                                              fill="#525F7F"/>
                                            <path
                                              d="M8.37868 3.29289L0 11.6716V14.5H2.82842L11.2071 6.12132L8.37868 3.29289Z"
                                              fill="#525F7F"/>
                                            </svg>
  ...
  <Modal v-show="modalActive && showModal == 'follow'" @close="closeModal">
    <template v-slot:body>
      ...
      <p class="font-inter-400 text-lg text-dark2 mb-11">By following this ticket you’ll be notified about any
        updates.</p>
    </template>
    <template v-slot:footer>
      <div class="flex items-center justify-end w-full">
        <button @click="closeModal"
                class="border border-blue mt-1 md:mt-0 min-w-110 w-full md:w-fit bg-white text-sm font-opensans-600 mr-4 py-2.5 px-5 text-blue rounded-lg flex items-center justify-center">
          Cancel
        </button>
        <button @click="followTicket(modalTicketID, pageNumber)"
                class="mt-1 md:mt-0 min-w-110 w-full md:w-fit bg-blue text-sm font-opensans-600 py-2.5 px-5 shadow-main text-white rounded-lg flex items-center justify-center">
          Follow Ticket
        </button>
      </div>
    </template>
  ...
  <Modal v-show="modalActive && showModal == 'unfollow'" @close="closeModal">
    <template v-slot:body>
      ...
      <h5 class="font-exo-600 text-xl text-dark2 mb-4">Follow Ticket #{{ modalTicketID }}</h5>
      <p class="font-inter-400 text-lg text-dark2 mb-11">By unfollowing this ticket you’ll no longer be notified about
        any updates. </p>
    </template>
    <template v-slot:footer>
      <div class="flex items-center justify-end w-full">
        ...
        <button @click="unfollowTicket(modalTicketID, pageNumber)"
                class="mt-1 md:mt-0 min-w-110 w-full md:w-fit bg-blue text-sm font-opensans-600 py-2.5 px-5 shadow-main text-white rounded-lg flex items-center justify-center">
          Unfollow Ticket
        </button>
      </div>
      ...
      <form onsubmit="return false" class="block">
        <div class="grid grid-cols-2 gap-y-6 gap-x-3">
          <div class="col-span-2">
            ...
            <div class="relative w-full">
              <div class="mt-2 w-full secondary-input" style="padding: 4px 0 0 0">
                <v-select :filter="fuseSearch" :options="products" :get-option-label="option => option.product_service"
                          placeholder="Choose" v-model="fProductSevice" :reduce="products => products.product_service">
          ...
          <div class="col-span-2">
            ...
            <div class="relative w-full">
              <div class="mt-2 w-full secondary-input" style="padding: 4px 0 0 0">
                <v-select :filter="fuseSearch" :options="severities" :get-option-label="option => option.name"
                          placeholder="Choose" v-model="fSeverity" :reduce="severities => severities.name">
          ...
          <div class="col-span-1">
            <div class="relative w-full">
              ...
              <input type="text" v-model="fFrom" placeholder="Month DD, YYYY" name="from" id="from"
          ...
          <div class="col-span-1">
            <div class="relative w-full">
              ...
              <input type="text" v-model="fTo" placeholder="Month DD, YYYY" name="to" id="to"
                     class="mt-2 w-full secondary-input"/>
              <img src="/img/icon/date-blue.svg" class="date-img">
            </div>
          </div>
          <div class="col-span-2">
            <div class="relative w-full">
              ...
              <input type="text" v-model="filterSearchAgency" placeholder="Search" name="selectagencies"
                     id="selectagencies" class="mt-4 w-full secondary-input" style="padding-left: 35px;"/>
  ...
  <Modal modalTitle="Ticket Status" v-show="modalActive && showModal == 'ticketStatusModal'" @close="closeModal">
    <template v-slot:body>
      <form onsubmit="return false" class="block">
        <div class="grid grid-cols-2 gap-y-6 gap-x-3">
          <div class="col-span-2">
            <p class="text-black font-inter-400 text-sm">Once updated, we’ll automatically send a notification to the
              assigned agencies & customer</p>
          </div>
          <div class="col-span-2">
            <div class="relative w-full">
              ...
              <div class="flex items-center justify-start my-3">
                <input type="radio" v-model="currentTicketStatus" value="ACKNOWLEDGED" id="acknowledged"
                       class="mr-2.5"/>
              ...
              <div class="flex items-center justify-start my-3">
                ...
                <label for="resolved" class="text-base text-blue-grey text-xs font-inter-700">Resolved <span
                  class="text-blue font-opensans-400">(0/2)</span></label>
              </div>
              <div class="flex items-center justify-start my-3">
          ...
          <div class="col-span-2">
            <div class="relative w-full">
              <label for="statusComment" class="text-base text-blue-grey text-xs font-inter-700">Status Update
                Comments</label>
              <textarea type="text" v-model="statusComment" placeholder="Write comment" name="statusComment"
                        id="statusComment" class="mt-2 w-full main-textarea"></textarea>
            </div>
            <div v-if="currentTicketStatus == 'RESOLVED' || currentTicketStatus == 'INVALID'"
                 class="info-warning mt-4 flex w-full rounded border border-light bg-greygrey p-4">
              <div class="block mr-4"><img src="/img/icon/info.png"></div>
              <p class="font-inter-400 text-black text-sm">Once you update this ticket to <b class="font-opensans-700">{{
                  currentTicketStatus
                }}</b>, you can no longer update the information</p>
            </div>
          </div>
        </div>
      </form>
    ...
    <template v-slot:footer>
      <div class="flex items-center justify-end w-full">
        ...
        <button @click.prevent="updateTicketStatus(modalTicketID, pageNumber)"
                class="mt-1 md:mt-0 min-w-110 w-full md:w-fit bg-blue text-sm font-opensans-600 mx-0 sm:mx-2 py-2.5 px-5 shadow-main text-white rounded-lg flex items-center justify-center">
          Update
        </button>
      </div>
