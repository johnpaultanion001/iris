    ...
    <PageLayout :pageName="title">
        <div class="grid grid-cols-12 gap-3 mb-6 pt-4">
                            ...
                            <div class="col-span-2">
                                <div class="relative w-full">
                                    <div class="mt-2 w-full secondary-input" style="padding: 4px 0 0 0">
                                        <v-select :filter="fuseSearch" :options="roles" :get-option-label="(option) => option.name" :placeholder="user.role" v-model="userrole" :reduce="roles => roles.name">
            ...
            <div class="col-span-12 col-start-1 md:col-span-10 col md:col-start-2 mt-5 flex items-center justify-end">
                <button @click="editUser()" class="mt-1 md:mt-0 w-full md:w-40 bg-blue text-sm font-opensans-600 mx-0 py-2.5 px-5 shadow-main text-white rounded-lg">
                    Update User
                </button>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async getUser(){
            ...
            this.firstname = response.data.data.name;
            this.lastname = response.data.data.lastname;
            this.useremail = response.data.data.email;
            this.usermobile = response.data.data.mobile_number;
            this.useragency = response.data.data.assigned_agencies.id;
        ...
        async editUser(){
            ...
            await axios.post('api/v1/user/'+this.id+'/update', inputs)
            .then((success) => {
